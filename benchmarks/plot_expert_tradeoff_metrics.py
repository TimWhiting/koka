...
from plot_utils import load_results_with_baselines, resolve_results_dir, prepare_tradeoff_data, get_tradeoff_color
...
metrics_to_plot = [
    {'Name': 'Value RIR (Strict)', 'Col': 'prec_val_rir_strict'},
    {'Name': 'Continuation RIR (Strict)', 'Col': 'prec_cont_rir_strict'},
]
...
df_final = load_results_with_baselines(resolve_results_dir())
...
def plot_combined_tradeoff(cost_name, suffix, xlabel):
    ...
    for metric_name, title, ax in metrics:
        ...
        for i, row in plot_df.iterrows():
            ...
            if has_base and has_new:
                ...
                ax.plot([p0[0], p1[0]], [p0[1], p1[1]], color=color, alpha=0.3, linewidth=3)
                ax.scatter(p0[0], p0[1], color='gray', s=50, marker='x', alpha=0.6, zorder=2)
                ax.scatter(p1[0], p1[1], color=color, s=55, marker='o', alpha=0.8, zorder=3)
            elif has_base:
                p0 = (row['Cost_Base'], row['Precision_Base'])
                ax.scatter(p0[0], p0[1], color='red', s=50, marker='x', alpha=0.8, zorder=2) # Base only
            elif has_new:
                p1 = (row['Cost_New'], row['Precision_New'])
                ax.scatter(p1[0], p1[1], color='green', s=55, marker='o', alpha=0.9, zorder=3) # New only
    ...
    legend_elements = [
        Line2D([0], [0], color='green', lw=5, label='Win-Win'),
        Line2D([0], [0], color='blue', lw=5, label='Trade-off'),
        Line2D([0], [0], color='red', lw=5, label='Regression'),
        Line2D([0], [0], color='gray', lw=5, label='Similar Precision'),
        Line2D([0], [0], marker='x', color='gray', label='1-kCFA', linestyle='None', markersize=10),
        Line2D([0], [0], marker='o', color='gray', label='H(1,1)', linestyle='None', markersize=10),
    ]
    fig.legend(handles=legend_elements, bbox_to_anchor=(1.02, 0.9), loc='upper left', borderaxespad=0.)
    
    plt.tight_layout(pad=0.2)
    outfile = f"benchmarks/images/plot_expert_tradeoff_combined{suffix}.png"
