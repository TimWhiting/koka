/-
  Concrete Big-Step Semantics for the locally-nameless ANF machine.

  This mirrors `DMCFA.Semantics`, but binders are resolved by *pushing*
  addresses onto `Env`'s stack (`Env.push`/`Env.pushMany`) rather than by
  extending a name → address map. Since pushing never overwrites an existing
  binding, none of the original `ρ x = none` "fresh in ρ" side-conditions are
  needed any more -- only store-freshness (`σ a = none`) remains, which is a
  genuine address-allocation invariant.
-/
...
/-- Resolve an atomic expression to a store address, if it denotes one
(`bvar`); other atomic forms don't denote addresses. -/
@[simp, grind =] def evalAddr (ae : AExp) (ρ : Env) : Option VAddr :=
  match ae with
  | .bvar i => ρ.lookup i
  | _ => none
...
@[simp, grind =] def findBranch (bs : Branches) (c : ConLabel) : Option Exp :=
  bs.findSome? fun b =>
    match b with
    | .branch c' e => if c' = c then some e else none
...
@[simp, grind =] def Handler.findOp (h : Handler) (op : OpName) : Option Exp :=
  h.opClauses.findSome? fun (op', e) => if op' = op then some e else none
...
inductive EvalExp : Exp → Env → Store → Value → Store → Prop where
  /-- [eval-let]: let _ = ce; e evaluates ce then continues with frame -/
  | eval_let :
      EvalCExp ce ρ σ v σ' →
...
inductive EvalCExp : CExp → Env → Store → Value → Store → Prop where
  ...
  /-- [eval-opApp]: operation application - arg resolves to an address, store unchanged -/
  | eval_opApp :
  ...
  /-- [eval-fun]: recursive function definition; the closure's own (fresh) address
  `a_v` is recorded as `selfAddr`, so call time pushes it as `bvar 0` (self),
  ...
  /-- [eval-match-succ]: match (succ _) branch with binding (reallocates) -/
  | eval_match_succ :
...
inductive ApplyKont : Kont → Denotable → Store → Value → Store → Prop where
  ...
  /-- [apply-restore]: restore let frame from continuation -/
  | apply_restore :
      ApplyKont κ d σ v' σ' →
      ContinueFrame (Frame.letFrame e ρ) v' σ' v'' σ'' →
...
inductive EvalExpN : Nat → Exp → Env → Store → Value → Store → Prop where
  | eval_let :
      ...
      ContinueFrameN n2 (Frame.letFrame e ρ) v σ' v' σ'' →
      EvalExpN (n1 + n2 + 1) (Exp.letE ce e l) ρ σ v' σ''
  | eval_tail :
...
inductive EvalCExpN : Nat → CExp → Env → Store → Value → Store → Prop where
  ...
  | eval_opApp :
      evalAddr ae ρ = some a_v →
      EvalCExpN 0 (CExp.opApp op ae) ρ σ (Value.suspended op [a_v] []) σ
  | eval_funApp_kont :
      evalAtomic f ρ σ = some (Denotable.kontClosure h ρ_h κ) →
      evalAtomic ae ρ σ = some d →
...
inductive ContinueFrameN : Nat → Frame → Value → Store → Value → Store → Prop where
  | continue_let :
      σ a_v = none →
      σ_let = σ.extend a_v d →
      EvalExpN n e (ρ.push a_v) σ_let v' σ' →
      ContinueFrameN (n + 1) (Frame.letFrame e ρ) (Value.den d) σ v' σ'
  | continue_op :
      ContinueFrameN 0 (Frame.letFrame e ρ) (Value.suspended op as_v κ) σ
                    (Value.suspended op as_v (Frame.letFrame e ρ :: κ)) σ
...
inductive HandleValueN : Nat → Handler → Env → Value → Store → Value → Store → Prop where
  | handle_return :
      ...
      σ a_v = none →
      σ_ret = σ.extend a_v d →
      EvalExpN n e_ret (ρ.push a_v) σ_ret v' σ' →
  ...
  | handle_op :
      ...
      σ a_arg = some d_arg →
      σ_op = (σ.extend a_vk (Denotable.kontClosure h ρ κ)).extend a_v' d_arg →
      EvalExpN n e_op ((ρ.push a_vk).push a_v') σ_op v σ' →
...
inductive ApplyKontN : Nat → Kont → Denotable → Store → Value → Store → Prop where
  ...
  | apply_restore :
      ApplyKontN n1 κ d σ v' σ' →
      ContinueFrameN n2 (Frame.letFrame e ρ) v' σ' v'' σ'' →
      ApplyKontN (n1 + n2 + 1) (Frame.letFrame e ρ :: κ) d σ v'' σ''
  | apply_restore_handle :
      ApplyKontN n1 κ d σ v' σ' →
      HandleValueN n2 h ρ v' σ' v'' σ'' →
...
theorem eval_cexp_to_N (h : EvalCExp ce ρ σ v σ') : ∃ n, EvalCExpN n ce ρ σ v σ' :=
  ...
  | .eval_fun h1 h2 h3 => ⟨0, .eval_fun h1 h2 h3⟩
  | .eval_match h1 h2 h3 h4 =>
    let ⟨n, hn⟩ := eval_exp_to_N h4
    ⟨n + 1, .eval_match h1 h2 h3 hn⟩
...
theorem eval_cexpN_to (h : EvalCExpN n ce ρ σ v σ') : EvalCExp ce ρ σ v σ' :=
  ...
  | .eval_fun h1 h2 h3 => .eval_fun h1 h2 h3
  | .eval_match h1 h2 h3 h4 => .eval_match h1 h2 h3 (eval_expN_to h4)
  | .eval_match_succ h1 h2 h3 h4 h5 h6 h7 =>
