/-
  ...
  Defines the substitution-based evaluation relation for B&P:
    c ⇓ r
  where c is a computation and r is a result (val e or # op e c, with c
  1-open: bvar 0 is the "resume slot").
-/
...
def BPHandler.hasOp (h : BPHandler) (op : OpName) : Bool :=
  h.opClauses.any fun (op', _) => op' = op

/-- Find operation clause body in handler (1-open: bvar 0 = op argument) -/
def BPHandler.findOp (h : BPHandler) (op : OpName) : Option Comp :=
  h.opClauses.findSome? fun (op', c) => if op' = op then some c else none
...
- Function application opens the body with the argument
...
inductive Eval : Comp → Result → Prop where
  ...
  /-- Lambda application: (fun → cbody) e ⇓ r if cbody.open_ e ⇓ r -/
  | app_lam :
      Eval (cbody.open_ e_arg) r →
      Eval (Comp.app (Expr.lam cbody) e_arg) r

  /-- let c1 in c2 ⇓ r when c1 ⇓ val e' and c2.open_ e' ⇓ r -/
  | let_val :
      Eval c1 (Result.value e') →
      Eval (c2.open_ e') r →
      Eval (Comp.letIn c1 c2) r

  /-- let c1 in c2 ⇓ # op e_op (let c_op in c2)
      when c1 ⇓ # op e_op c_op -/
  | let_op :
      Eval c1 (Result.op op e_op c_op) →
      Eval (Comp.letIn c1 c2) (Result.op op e_op (Comp.letIn c_op c2))

  /-- let rec c1 in c2 ⇓ r
      when c2.open_ (fun → let rec c1 in c1) ⇓ r -/
  | let_rec :
  ...
  /-- match 0 with 0 → c1 | succ → c2 ⇓ r when c1 ⇓ r -/
  | match_zero :
      Eval c1 r →
      Eval (Comp.matchNat Expr.zero c1 c2) r

  /-- match (succ e') with 0 → c1 | succ → c2 ⇓ r when c2.open_ e' ⇓ r -/
  | match_succ :
      Eval (c2.open_ e') r →
      Eval (Comp.matchNat (Expr.succ e') c1 c2) r

  /-- with h handle c ⇓ r when c ⇓ val e' and h.returnClause.open_ e' ⇓ r -/
  | handle_val :
      Eval c (Result.value e') →
      Eval (h.returnClause.open_ e') r →
      Eval (Comp.withHandle h c) r

  /-- with h handle c ⇓ r when c ⇓ # op e_op c_op and op ∈ h
      and the op clause body `c_h` (binds 2: bvar 0 = arg, bvar 1 = resume),
      opened with the argument `e_op` and the resumption
      `fun → with h handle c_op`, evaluates to r. -/
  | handle_op :
  ...
  /-- with h handle c ⇓ # op e_op (with h handle c_op)
      when c ⇓ # op e_op c_op and op ∉ h -/
  | handle_forward :
      Eval c (Result.op op e_op c_op) →
      h.hasOp op = false →
      Eval (Comp.withHandle h c) (Result.op op e_op (Comp.withHandle h c_op))

  /-- op e ⇓ # op e (val (bvar 0)) -/
  | op_call :
      Eval (Comp.opCall op e) (Result.op op e (Comp.val (Expr.bvar 0)))
...
inductive EvalN : Nat → Comp → Result → Prop where
  ...
  | app_lam : EvalN n (cbody.open_ e_arg) r →
      EvalN (n+1) (Comp.app (Expr.lam cbody) e_arg) r
  | let_val : EvalN n1 c1 (Result.value e') → EvalN n2 (c2.open_ e') r →
      EvalN (n1+n2+1) (Comp.letIn c1 c2) r
  | let_op : EvalN n c1 (Result.op op e_op c_op) →
      EvalN (n+1) (Comp.letIn c1 c2) (Result.op op e_op (Comp.letIn c_op c2))
  | let_rec : EvalN n (c2.open_ (Expr.lam (Comp.letRec c1 c1))) r →
      EvalN (n+1) (Comp.letRec c1 c2) r
  ...
  | match_zero : EvalN n c1 r → EvalN (n+1) (Comp.matchNat Expr.zero c1 c2) r
  | match_succ : EvalN n (c2.open_ e') r →
      EvalN (n+1) (Comp.matchNat (Expr.succ e') c1 c2) r
  | handle_val : EvalN n1 c (Result.value e') →
      EvalN n2 (h.returnClause.open_ e') r →
      EvalN (n1+n2+1) (Comp.withHandle h c) r
  | handle_op : EvalN n1 c (Result.op op e_op c_op) →
      h.findOp op = some c_h →
      EvalN n2 (compOpenRec 0 e_op
            (compOpenRec 1 (Expr.lam (Comp.withHandle h c_op)) c_h)) r →
      EvalN (n1+n2+1) (Comp.withHandle h c) r
  | handle_forward : EvalN n c (Result.op op e_op c_op) → h.hasOp op = false →
      EvalN (n+1) (Comp.withHandle h c) (Result.op op e_op (Comp.withHandle h c_op))
  | op_call : EvalN 0 (Comp.opCall op e) (Result.op op e (Comp.val (Expr.bvar 0)))
...
theorem eval_to_evalN : Eval c r → ∃ n, EvalN n c r := by
  ...
  | handle_val _ _ ih1 ih2 =>
    obtain ⟨n1, hn1⟩ := ih1; obtain ⟨n2, hn2⟩ := ih2
    exact ⟨n1+n2+1, .handle_val hn1 hn2⟩
...
theorem evalN_to_eval : EvalN n c r → Eval c r := by
  ...
  | handle_val _ _ ih1 ih2 => exact .handle_val ih1 ih2
