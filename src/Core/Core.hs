...
isDefNonRec :: DefGroup -> Bool
isDefNonRec (DefNonRec _) = True
isDefNonRec _             = False
...
instance Functor CPState where
    fmap f (CPState x u d) = CPState (f x) u d
...
liftCorePhaseRes :: (Int -> DefGroups -> (a,Int)) -> CorePhase b a
liftCorePhaseRes f
