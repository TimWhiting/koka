...
import Compiler.Module (Loaded (..), Module (..), ModStatus (LoadedIface, LoadedSource), addOrReplaceModule, moduleNull)
...
findAllUsage first expr@Var{varName=tname@TName{getName = name}} ctx env = do
  case ctx of
    ModuleC{} -> do
      loaded <- loadedModules . loaded <$> getState
      let lds = mapMaybe (\m -> if modStatus m == LoadedSource then Just m else Nothing) loaded
      foldM (\res m -> do
...
runEvalQueryFromRangeSource :: Loaded -> (Module -> IO Module) -> (Range, RangeInfo) -> Module -> AnalysisKind -> Int -> ([(EnvCtx, ([UserExpr], [UserDef], [Syn.Lit], [String], Set Type, Set String))], Loaded)
runEvalQueryFromRangeSource ld loadModuleFromSource rng mod kind m =
  let (r, lattice, Just x) = runQueryAtRange ld loadModuleFromSource rng mod kind m $ \exprctxs ->
        case exprctxs of
          exprctx:rst -> do
...
runQueryAtRange :: Loaded -> (Module -> IO Module) -> (Range, RangeInfo) -> Module -> AnalysisKind -> Int -> ([ExprContext] -> FixDemand x () ()) -> (FixOutput, M.Map FixInput (BasicLattice AFixOutput), Maybe x)
runQueryAtRange loaded loadModuleFromSource (r, ri) mod  kind m query =
  let cid = ExprContextId (-1) (modName mod)
      ...
      result = runFix (focalContext >>= query) (DEnv m kind modCtx modCtx (EnvTail TopCtx) "" "" loadModuleFromSource ()) (State loaded M.empty 0 M.empty 0 Nothing M.empty ())
  in case result of
