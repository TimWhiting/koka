...
chkExpr expr
  = case expr of
      ...
      App (TypeApp (Var tname _) _) _ _ | getName tname `elem` [nameCCtxSetCtxPath,nameLazyMemoizeTarget,nameLazyEnter,nameLazyLeave]
        -> return ()
      App (TypeApp (Var tname _) _) [_, conApp] _ | getName tname == nameLazyMemoize
        -> chkExpr conApp
...
chkLazyCon (TName cname _ _) repr
  = do fip <- getFip
