...
module Core.Demand.DemandMonad(
  ...
  focusParam, focusBody, focusChild, focusFun, focusScrutinee, focusBranch, focusLetBod, enterBod, succAEnv,
...
import Data.List (find, intercalate, elemIndex)
...
useGas g = do
  st <- getState
  let newGas = gas st - g
  -- trace ("New gas " ++ show newGas ++ " old gas " ++ show (gas st)) $ return ()
  if newGas < 0 && (gas st /= (-1)) then
    do
      trace ("Out of gas " ++ show g ++ " " ++ show (gas st)) $ return ()
...
withGas f = do
  allout <- decGas
  if allout then error "All out of gas" else f
...
instance Lattice FixOutput AFixChange where
  bottom = N
  insert (FA a) N =
    let (change, res) = addChange emptyAbValue a
    in (FA change, A res)
  insert (FA a) (A b) =
    let (change, res) = addChange b a
    in (FA change, A res)
  insert (FE e) N = (FE e, E $ S.singleton e)
  insert (FE e) (E e1) = (FE e, E (S.insert e e1))
  lte B N = True
...
focusScrutinee :: ExprContext -> FixDemandR x s e ExprContext
focusScrutinee = focusChild 0

focusBranch :: Int -> ExprContext -> FixDemandR x s e ExprContext
...
getTopDefCtx ctx@(ModuleC{}) name = do
  -- trace ("Getting top def ctx " ++ show name ++ " " ++ show ctx) $ return ()
  children <- childrenContexts ctx
  let findCtx :: [ExprContext] -> FixDemandR x s e ExprContext
...
childrenOfDef ctx def =
  case def of
    C.DefNonRec d -> do
      grp <- addContextId (\newId -> DefCGroup newId ctx [defTName d] def)
      bind <- addContextId (\newId -> DefCNonRec newId grp (defTName d))
      addChildrenContexts (contextId grp) [bind]
      return [grp]
    C.DefRec ds -> do
      let tnames = map defTName ds
      grp <- addContextId (\newId -> DefCGroup newId ctx tnames def)
      bindings <- zipWithM (\i d -> addContextId (\newId -> DefCRec newId grp i tnames)) [0..] ds
      addChildrenContexts (contextId grp) bindings
      return [grp]
...
childrenContexts :: HasCallStack => ExprContext -> FixDemandR x s e [ExprContext]
