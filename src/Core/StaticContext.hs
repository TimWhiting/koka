...
showSimpleContext ctx =
  ...
  case ctx of
    ModuleC{} -> "Module " ++ r
    DefCRec _ _ _ i d -> "DefRec(" ++ show (defTName (defOfCtx ctx)) ++ ")"
    DefCNonRec _ _ _ d -> "DefNonRec(" ++ show (defTName (defOfCtx ctx)) ++ ")"
    LamCBody _ _ tn _-> "LamBody(" ++ show tn ++ ")" 
    AppCLambda{} -> "AppLambda(" ++ rmNL (exprOfCtx ctx) ++ ")"
    AppCParam{} -> "AppParam(" ++ rmNL (exprOfCtx ctx) ++ ")"
    LetCDef _ _ _ i d -> "LetDef(" ++ show (defTName (defOfCtx ctx)) ++ ")"
    LetCBody{} -> "LetBody(" ++ rmNL (exprOfCtx ctx) ++ ")"
    CaseCMatch{} -> "CaseMatch(" ++ rmNL (exprOfCtx ctx) ++ ")"
    CaseCBranch{} -> "CaseBranch(" ++ rmNL (exprOfCtx ctx) ++ ")"
    ExprCBasic{} -> "ExprBasic(" ++ rmNl (show (exprOfCtx ctx)) ++ ")"
    ExprCTerm{} -> "Query Error"

rmNl :: String -> String
rmNl s = s -- T.unpack $ T.replace "\n" " newline " (T.pack s)
...
defOfCtx ctx = 
  case ctx of
    ...
    LetCDef _ _ _ i dg -> defsOf dg !! i
