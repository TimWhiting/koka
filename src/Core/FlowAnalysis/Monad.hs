...
focusScrutinee :: ExprContext -> FixAR x s e i o c ExprContext
...
getTopDefCtx ctx@(ModuleC{}) name = do
  ...
  let findCtx :: [ExprContext] -> FixAR x s e i o c ExprContext
...
bindExternal2 :: Expr -> FixAR r s e i o c (Maybe ExprContext)
bindExternal2 var@(Var tn@(TName name tp _) vInfo) = do
