...
instance Show FixChange where
  show (AC x) = show x
  show (KKC l k kclos) = show (l, k, kclos)
...
instance Lattice FixOutput FixChange where
  ...
  isBottom _ = False
  insert (AC a) Bottom = (AC a, A $ snd $ addChange emptyAbValue a)
  insert (AC a) (A x) = mapChange (addChange x a) AC A
  insert (KC l k) (K x) = (KC l k, K $ S.insert (l, k) x)
  insert (KC l k) Bottom = (KC l k, K $ S.singleton (l, k))
  insert (KKC l k kclos) (KK x) = (KKC l k kclos, KK $ S.insert (l, k, kclos) x)
  insert (KKC l k kclos) Bottom = (KKC l k kclos, KK $ S.singleton (l, k, kclos))
  insert (CC l k c) (C x) = (CC l k c, C $ S.insert (l, k, c) x)
  insert (CC l k c) Bottom = (CC l k c, C $ S.singleton (l, k, c))
  insert (BC b) (B x) = (BC b, B $ S.insert b x)
  insert (BC b) Bottom = (BC b, B $ S.singleton b)
