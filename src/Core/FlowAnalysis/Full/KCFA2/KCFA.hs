...
module Core.FlowAnalysis.Full.KCFA2.KCFA where
...
import Core.FlowAnalysis.Full.KCFA2.Primitives
    ...
    ROp dval ctx' frame' dframe' knext -> do
      case dval of
        ...
        DVal hName opName oExpr [newAddr] | hName == getName varTName && opName == nameLocalSet -> do
          ...
          RV (res, newCtx) <- apply knext UnitAddr xctx
        ...
        case lookup opName ops' of
          ...
          Just op -> do
            ...
            if isTailOp opConName then do -- TODO: Add operation call context?
              ...
              extendStore (BindingAddr newCtx (last params) (contextId op)) (AChangeKont knext venv h)
              eval opBod (limitEnv newEnv (fvs opBod)) newCtx
      else do
