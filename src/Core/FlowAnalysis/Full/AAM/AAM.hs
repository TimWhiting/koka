...
lamAddrs lam (KTime label contour) =
  let names = lamNames lam
      addrs = zip3 names (repeat (contextId lam)) (repeat contour)
  in (names, addrs)

allocBindAddrs :: HasCallStack => DefGroup -> ExprContext -> Time -> FixAAMR r s e [Addr]
allocBindAddrs (DefRec defs) expr (KTime label contour) = do
  let names = dfsTNames defs
  return $ zip3 names (repeat $ contextId expr) (repeat contour)
allocBindAddrs (DefNonRec df) expr (KTime label contour) = 
  return [(TName (defName df) (defType df) Nothing, contextId expr, contour)]
...
alloc (Cont (AChangeKont k kr) store kstore kont mkont (KTime label contour)) (AppL nargs e0 env') = do
  let names = [TName (newHiddenName $ "k" ++ show x) typeAny Nothing | x <- [0..nargs - 1]]
  return $ zip3 names (repeat $ contextId e0) (repeat contour)
alloc (Cont (AChangeClos lam env) store kstore kont mkont (KTime label contour)) (AppL nargs e env') = do
  let names = lamNames lam
  if length names /= nargs then doBottom -- error ("alloc: " ++ show names ++ " " ++ show nargs)
  else do
    let addrs = repeat (contextId lam)
    return $ zip3 names addrs (repeat (contour))
alloc (Cont (AChangePrim n expr _) store kstore kont mkont (KTime label contour)) (AppL nargs e0 env') = do
  let addrs = repeat $ contextId e0
      names =  map (\x -> TName (newHiddenName $ nameStem (getName n) ++ show x) typeAny Nothing) (take nargs [0..])
  return $ zip3 names addrs (repeat contour)
alloc (Cont (AChangeConstr con env) store kstore kont mkont (KTime label contour)) (AppL nargs e env') = do
    ...
    Con tn _ _ -> do
      ...
      case splitFunScheme tp of
        Just (_, args, _, _) ->
          ...
          else return $ zip3 (map (\(n,t) -> TName n t Nothing) args) addrs (repeat contour)
        Nothing -> return [(tn, contextId e, contour)]
...
toClause n (AChangeClos clos env) = AChangeConstr clos env
newTime :: Time -> FixInput -> Maybe Frame -> Time

newTime (KTime local old) (Cont (AChangeClos _ _) _ _ _ _ _) (Just (AppR{})) = KTime Nothing (integrate local old)
newTime (KTime local old) (Cont{}) (Just (OpR{})) = KTime Nothing (integrate local old)
newTime time (Cont{}) _ = time
newTime old@(KTime _ k) (Eval e1 _ _ _ _ _ _) Nothing = 
...
doStep i =
  memo i $ do
    ...
    case i of
      Eval expr env store kstore kont mkont time -> do
        let time' = 
              case kont of
                [] -> time
        ...
        case exprOfCtx expr of
          Lit l -> doGC $ Cont (injLit l env) store kstore kont mkont time
          Var x _ -> do
            if isPrimitive x then do
              doGC $ Cont (AChangePrim x expr env) store kstore kont mkont time'
            else do
              -- trace ("Var: " ++ show x ++ " " ++ show env) $ return ()
              -- trace ("Var: " ++ show (contextOf expr)) $ return ()
              v <- storeLookup x env store
              doGC $ Cont v store kstore kont mkont time'
          Con _ _ _ -> do
            doGC $ Cont (AChangeConstr expr env) store kstore kont mkont time'
          App (TypeApp (Var name _) tps@[_, _, tp, _, _]) args _ | nameHandle == getName name -> do
            ...
            doGC $ Eval c env store  (kstoreExtend kaddr kont kstore) [HandleL [] tp expr env,EndHandle (KontEnd mkont)] mkont time'
          App (TypeApp (Var name _) _) args _ | nameEffectOpen == getName name -> do
            f <- focusChild 1 expr
            doGC $ Eval f env store kstore kont mkont time'
          App (TypeApp (Var name _) _) args _ | isNamePerform $ getName name -> do
            ...
            doGC $ Eval select env store (kstoreExtend kaddr kont kstore) [OpL expr env n eff kaddr] mkont time'
          App f args _ -> do
            f <- focusChild 0 expr
            doGC $ Eval f env store kstore (AppL (length args) expr env:kont) mkont time'
          Lam args eff body -> do
            doGC $ Cont (AChangeClos expr (limitEnv env (fvs expr))) store kstore kont mkont time'
          ...
          TypeApp (Var _ _) _ -> do
            ...
            doGC $ Eval ex env store kstore kont mkont time'
          TypeApp (Con _ _ _) _ -> do
            ex <- focusChild 0 expr
            doGC $ Eval ex env store kstore kont mkont time'
          Case [e] bs -> do
            ex <- focusChild 0 expr
            doGC $ Eval ex env store kstore (CaseR expr env:kont) mkont time'
          Let binds body -> do
            ...
            doGC $ Eval ex env' store kstore (LetL 0 (length binds) 0 (length $ defsOf bg) addrs expr env':kont) mkont time'
          _ -> error $ "doStep: " ++ show expr ++ " not handled"
      Cont achange store kstore kont mkont time@(KTime label contour) -> do
        ...
        case kont of
          [OpL expr env n eff kaddr] -> do
            ...
            doGC $ Eval child senv store kstore [OpL1 expr env n eff kaddr] mkont time'
          [OpL1 expr env n eff kaddr] -> do
            if n == 0 then do
              handframes <- getHandler eff kaddr kstore
              let (kont, handler@(HandleR [tag, hnd, ret, act] ef e p):metakont) = handframes
              ...
              doGC $ Eval body env' store' kstore (OpR expr [] eff kont:handler:metakont) mkont time'
            else do
              arg <- focusChild 3 expr
              doGC $ Eval arg env store kstore [OpL2 expr env n achange [] eff kaddr] mkont time'
          [OpL2 expr env n ch achanges eff kaddr] -> do
            if n == length achanges + 1 then do
              (kont, handler@(HandleR [tag, hnd, ret, act] ef e p):metakont) <- getHandler eff kaddr kstore
              let AChangeClos select senv = ch
              body <- focusBody select -- Body of the select 
              -- trace ("OpL2=l" ++ show n ++ ": " ++ show body) $ return ()
              let (names, addrs) = lamAddrs select time
                  (env', store') = extend env store [head names] [head addrs] [hnd]
              doGC $ Eval body env' store' kstore (OpR expr (achanges ++ [achange]) eff kont:handler:metakont) mkont time'
            else do
              -- trace ("OpL2: " ++ show (length achanges)) $ return ()
              arg <- focusChild (4 + length achanges) expr
              doGC $ Eval arg env store kstore [OpL2 expr env n ch (achanges ++ [achange]) eff kaddr] mkont time'
          [OpR expr changes eff kaddr,hndframe@(HandleR [tag, hnd, ret, act] ef e p), EndHandle kend] -> do 
            ...
            let AChangeConstr op openv = achange
            opbod <- focusBody op
            ...
            --   doGC $ Eval opbod env' store' kstore [KResume kaddr,hndframe,EndHandle kend] mkont time'
            ...
            --       changes' = changes ++ [AChangeKont kaddr mkont] -- TODO: Fix this
            --       (env', store') = extend openv store names addrs' changes'
            --   doGC $ Eval opbod env' store' kstore [EndHandle kend] mkont time'
            -- else
            --   error "OpR"
            error "OpR"
          HandleR [tag, hnd, retClos, act] ef e p:ls -> do
            ...
            body <- focusBody ret
            doGC $ Eval body env' store' kstore ls mkont time' 
          HandleL changes ef e p:ls -> do
            ...
            if length changes == 3 then do
              ...
              doGC $ Eval body env store (kstoreExtend kaddr (HandleR (changes ++ [achange]) ef e p:ls) kstore) [EndCall kaddr] mkont time'
            else do
              nextparam <- focusChild (length changes + 2) e
              doGC $ Eval nextparam p store kstore (HandleL (changes ++ [achange]) ef e p:ls) mkont time'
          l@(AppL n e p):ls -> do
            if n == 0 then do
              let AChangeClos lam env = achange
              body <- focusBody lam
              let kaddr = KAddr (lam, env, time)
              doGC $ Eval body env store (kstoreExtend kaddr ls kstore) [EndCall kaddr] mkont time'
            else if n == 1 then do
              arge <- focusChild 1 e
              addrs <- alloc i l
              -- trace ("AppL: " ++ show n ++ " " ++ show arge ++ " " ++ show p) $ return ()
              -- k' <- 
              doGC $ Eval arge p store kstore (AppR achange addrs e:ls) mkont time'
            else do
              arge <- focusChild 1 e
              -- trace ("AppL: " ++ show n ++ " " ++ show arge ++ " " ++ show p) $ return ()
              addrs <- alloc i l
              doGC $ Eval arge p store kstore (AppM achange addrs e 1 n p:ls) mkont time'
          AppM clos addrs e n t p:ls -> do
            ...
            if n + 1 == t then do
              ...
              doGC $ Eval arge p store' kstore (AppR clos addrs e:ls) mkont time'
            else do
              doGC $ Eval arge p store' kstore (AppM clos addrs e (n + 1) t p:ls) mkont time'
          (AppR c@(AChangePrim p clos env) addrs e):ls | isNameClause (getName p) -> do
            -- trace ("ClauseTail: " ++ show c ++ " " ++ show ls) $ return ()
            doGC $ Cont (toClause (getName p) achange) store kstore ls mkont time'
          (AppR c@(AChangeKont k kaddr) addrs e):ls -> do
            -- trace ("Call resume:" ++ show k) $ return ()
            case M.lookup k kstore of
              Just sframes -> do
                frames <- each $ map return (S.toList sframes) -- TODO: GC the kstore prior to extending it
                doGC $ Cont achange store (kstoreExtend kaddr ls kstore) frames mkont time'
          (AppR c@(AChangeClos lam env) addrs e):ls -> do
            -- trace ("AppR: " ++ show c ++ " " ++ show ls) $ return ()
            body <- focusBody lam
            let args = lamNames lam
                store' = extendStore store (last addrs) achange
                kaddr = KAddr (lam, env, time)
            -- trace ("AppR: " ++ show c ++ " " ++ show ls) $ return ()
            doGC $ Eval body (extendEnv env args addrs) store' (kstoreExtend kaddr ls kstore) [EndCall kaddr] mkont time'
          (AppR ccon@(AChangeConstr con env) addrs e):ls -> do
            case exprOfCtx con of
              Con _ _ _ -> do
                ...
                doGC $ Cont (AChangeConstr con env') store' kstore ls mkont time'
          (AppR cprim@(AChangePrim p ctx env) addrs e):ls -> do
            ...
            doGC $ Cont res store' kstore ls mkont time'
          LetL bgi bgn bi bn addrs e p:ls -> do
            ...
            if bgi + 1 == bgn && bi + 1 == bn then do
              ...
              body <- focusChild 0 e
              doGC $ Eval body (extendEnv p (dgsTNames dgs) addrs) store' kstore ls mkont time'
            else if bi + 1 /= bn then do
              -- trace ("LetL In Group") $ return ()
              bind <- focusLetDefBinding bgi bi e
              doGC $ Eval bind p store' kstore (LetL bgi bgn (bi + 1) bn addrs e p:ls) mkont time'
            else if bi + 1 == bn then do
              ...
              doGC $ Eval bind env' store' kstore (LetL bgi' bgn 0 (length $ defsOf bg) addrs' e p:ls) mkont time'
            else
              error "Cont LetL"
                  ...
                  matchBranches achange ((Branch [pat] body, n):bs) = do
                    ...
                    case matches of
                      Just (venv, vstore) -> do
                        ...
                        doGC $ Eval body venv vstore kstore ls mkont time'
          ...
          [EndCall k] -> do
            ...
            doGC $ Cont achange store kstore l mkont time'
          [EndHandle (KontEnd k)] -> do
            let res = M.lookup k kstore
            l <- each $ map return (maybe [] S.toList res)
            doGC $ Cont achange store kstore l mkont time'
          [EndHandle (KontFrame k)] -> do
            let res = M.lookup k kstore
            l <- each $ map return (maybe [] S.toList res)
            doGC $ Cont achange store kstore l mkont time'
...
getHandler :: HasCallStack => Effect -> KAddr -> KStore -> FixAAMR r s e (KAddr, [Frame])
...
getHandlerF :: HasCallStack => Effect -> [Frame] -> KStore -> FixAAMR r s e (KAddr, [Frame])
getHandlerF eff frames kstore = do
  ...
  case frames of
    ...
    [EndHandle (KontEnd k)] -> getHandler eff k kstore
    [EndHandle (KontFrame k)] -> getHandler eff k kstore
