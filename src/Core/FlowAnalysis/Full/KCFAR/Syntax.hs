...
extractMetrics (cache, exprIds) (cacheExpected, _) =
  let
    ...
    exprToValSemSizes = M.map semSizeOf $ M.fromListWith (<>) [ (fromJust (M.lookup (contextId c) exprIds), resolveRValue (S.map fst vs)) | (Step (CEval c _), RValue vs) <- M.toList cache, not $ onlyLit (resolveRValue (S.map fst vs))]
    exprToValStrSizes = M.map abStructuralSize $ M.fromListWith (<>) [ (fromJust (M.lookup (contextId c) exprIds), resolveRValue (S.map fst vs)) | (Step (CEval c _), RValue vs) <- M.toList cache , not $ onlyLit (resolveRValue (S.map fst vs))]
    ...
    callTargetSemSizes = M.map semSizeOf $ M.fromListWith (<>) [ (fromJust (M.lookup (contextId c) exprIds), resolveRValue (S.map fst vs)) | (Step (CEval c _), RValue vs) <- M.toList cache, isIndirectAppFun c, not $ onlyLit (resolveRValue (S.map fst vs))]
    callTargetStrSizes = M.map abStructuralSize $ M.fromListWith (<>) [ (fromJust (M.lookup (contextId c) exprIds), resolveRValue (S.map fst vs)) | (Step (CEval c _), RValue vs) <- M.toList cache, isIndirectAppFun c, not $ onlyLit (resolveRValue (S.map fst vs)) ]
