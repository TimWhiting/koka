...
module Core.FlowAnalysis.Full.DMCFAROld.AbstractValue where
...
instance Show Addr where
  ...
  show (ImplicitLRAddr ctx nm ctxId) = "ILR@(" ++ showSimpleCtxId ctxId ++ ":" ++ show nm ++ ":" ++ show ctx ++ ")"
...
instance Show AChange where
  ...
  show (AChangeObj exp name args) = show name ++ "(" ++ show args ++ ")"
