...
module Core.FlowAnalysis.Full.DMCFA2.Syntax where
...
import Core.FlowAnalysis.Full.DMCFA2.DMCFA
            ...
            (AProgram name mainCtx resCtx):rest ->
              do
                result <- do
                  ...
                  let dir = "benchmarks/results/dmcfae2/" ++ show d ++ "/" ++ show m ++ "/" ++ nameModule (modName mod)
                  ...
                  case mbRes of
                    Just (l, analysisResult, times) -> do
                      ...
                      let value = PolyVariantMetrics "dmcfae2" d m (nameModule (modName mod) ++ "/" ++ name) times False (Just metrics)
                    ...
                    Nothing -> do
                      let value = PolyVariantMetrics "dmcfae2" d m (nameModule (modName mod) ++ "/" ++ name) [] True Nothing
                      BS.writeFile (dir ++ "/" ++ name ++ ".json") (encode (toJSON value))
...
extractMetrics (cache, exprIds) (cacheExpected, _) =
  let
    ...
    -- Aggregate all value sets by their vAddrId (0CFA key)
    ...
    -- Combined 0CFA precision (total singletons / total 0CFA addresses)
    ...
    applyContStrSizes = M.map S.size $ M.fromListWith (<>) [ (show $ kAddrId exprIds c, S.map ((kAddrId exprIds) . snd) vs) | (KStore c, KValue vs) <- M.toList cache ]
...
evalMain2 :: BuildContext
  -> TypeChecker -> Module -> Int -> Int
  -> IO Bool
evalMain2 bc build mod m d = do
