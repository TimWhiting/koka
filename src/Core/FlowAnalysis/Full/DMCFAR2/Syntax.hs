...
module Core.FlowAnalysis.Full.DMCFAR2.Syntax where
...
import Core.FlowAnalysis.Full.DMCFAR2.DMCFA
            ...
            (AProgram name mainCtx resCtx):rest ->
              do
                result <- do
                  ...
                  let dir = "benchmarks/results/dmcfar2/" ++ show d ++ "/" ++ show m ++ "/" ++ nameModule (modName mod)
                  ...
                  case mbRes of
                    Just (l, analysisResult, times) -> do
                      ...
                      let value = PolyVariantMetrics "dmcfar2" d m (nameModule (modName mod) ++ "/" ++ name) times False (Just metrics)
                    ...
                    Nothing -> do
                      let value = PolyVariantMetrics "dmcfar2" d m (nameModule (modName mod) ++ "/" ++ name) [] True Nothing
                      BS.writeFile (dir ++ "/" ++ name ++ ".json") (encode (toJSON value))
...
extractMetrics (cache, exprIds) (cacheExpected, _) =
  let
    ...
    exprToValSemSizes = M.map semSizeOf $ M.fromListWith (<>) [ (fromJust (M.lookup (contextId c) exprIds), resolveRValue vs) | (Step (CEval c _), RValue vs) <- M.toList cache, not $ onlyLit (resolveRValue vs)]
    exprToValStrSizes = M.map abStructuralSize $ M.fromListWith (<>) [ (fromJust (M.lookup (contextId c) exprIds), resolveRValue vs) | (Step (CEval c _), RValue vs) <- M.toList cache , not $ onlyLit (resolveRValue vs)]
    ...
    callTargetSemSizes = M.map semSizeOf $ M.fromListWith (<>) [ (fromJust (M.lookup (contextId c) exprIds), resolveRValue vs) | (Step (CEval c _), RValue vs) <- M.toList cache, isIndirectAppFun c, not $ onlyLit (resolveRValue vs)]
...
evalMainR2 :: BuildContext
  -> TypeChecker -> Module -> Int -> Int
  -> IO Bool
evalMainR2 bc build mod m d = do
