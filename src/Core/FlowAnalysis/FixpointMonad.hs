...
module Core.FlowAnalysis.FixpointMonad(
  ...
  runExample, writeDependencyGraph
  ) where
...
instance (Ord a) => Ord (SimpleChange a) where
  LChangeTop `compare` _ = GT
...
instance (Ord a) => Lattice (SimpleLattice a) (SimpleChange a) where
  bottom = LBottom
  insert (LChangeSingle a) b = 
    case b of
      LBottom -> (LChangeSingle a, LSingle a)
      LSingle b' -> if a == b' then (LChangeSingle a, LSingle a) else (LChangeTop, LTop)
      LTop -> (LChangeTop, LTop)
  insert LChangeTop b = 
      (LChangeTop, LTop)
  lte _ LTop = True
...
instance Ord a => Lattice (ChangeSet a) a where
  ...
  insert a (ChangeSet s) = (a, ChangeSet $ S.insert a s)
...
writeDependencyGraph modName cache = do
  ...
  writeFile ("scratch/debug/" ++ modName ++ ".dot") dot
  return ()
