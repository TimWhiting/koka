...
module Core.FlowAnalysis.DDMCFA.DemandAnalysis(
...
findUsages tname ctx env = 
  ...
  let searchChildren =  
        visitEachChild ctx $ do
          ...
          findUsage tname childCtx env
      recur =
        visitEachChild ctx $ do
          -- visitChildrenCtxs sets the currentContext
          childCtx <- currentContext <$> getEnv
          findUsages tname childCtx env
  in case ctx of
       ModuleC{} -> recur -- Module definitions
       DefCGroup{} -> recur -- Top level definitions
       ...
       DefCNonRec{} -> searchChildren -- Non recursive top level definitions
       _ -> 
        case contextOf ctx of
          Just (LetCDefGroup{}) -> searchChildren
          _ -> error $ "findUsages: Searching for usages starting in a bad context " ++ show ctx
  ...
  let nameEq = (== name)
      ...
      childrenUsages = do
        ...
        visitEachChild ctx $ do
          -- visitChildrenCtxs sets the currentContext
          childCtx <- currentContext <$> getEnv
          findUsage tname childCtx env in
    case ctx of
      ExprCBasic _ c (Var{varName=TName{getName=name2}}) ->
        if nameEq name2 then do
          query <- getQueryString
          trace (query ++ "Found usage of " ++ show name ++ " in " ++ show (ppContextPath ctx)) $ return ()
          return (c, env)
        else
    ...
    Nothing -> 
      case ctx of
        DefCGroup{} -> do
          ...
          doEval (ctx', env) query
        LetCDefGroup{} -> do
          ctx' <- focusChild 0 ctx
          doEval (ctx', env) query
        _ -> error "doEval: can't find expression"
    Just expr ->
      case expr of
        Lam{} -> -- LAM CLAUSE
        ...
        v@(Var tn _) -> do -- REF CLAUSE
          -- trace (query ++ "REF: " ++ show tn) $ return ()
          ...
          if prim then do
            ...
            case binded of
              ...
              BoundDef ctx boundEnv index -> do
                -- trace (query ++ "REF-def: " ++ show parentCtx ++ " " ++ show index) $ return []
                -- For a top level definition, it evaluates to itself
                ctx <- focusChild index ctx
                qeval (ctx, boundEnv)
              BoundLetDef ctx boundEnv index -> do
                trace (query ++ "LetDef: " ++ show ctx ++ " " ++ show index) $ return []
                -- For a top level definition, it evaluates to itself
                children <- childrenContexts ctx
                trace (query ++ "LetDef: " ++ show children) $ return []
                ctx <- focusChild index ctx
                qeval (ctx, boundEnv)
              BoundCase caseCtx caseEnv branchIndex patBinding -> do
                -- For a case bound name, we focus the scrutinee and delegate to a special function to handle this recursively for nested patterns
                mscrutinee <- focusScrutinee (fromJust $ contextOf caseCtx)
        ...
        Case expr branches -> do
          e <- focusScrutinee ctx
          -- trace (query ++ "CASE depend: " ++ show e ++ " " ++ show env) $ return []
          res <- qeval (e, env)
        ...
        case kind of
          BasicEnvs -> do
            ...
            if cc1 == cc0 then do
              ...
              trace (query ++ "INCOMPATIBLE CALLS:\n\nFIRST:" ++ show cc1 ++ "(" ++ show env ++ ")" ++ "\n\nSECOND:" ++ show cc0 ++ "(" ++ show callenv ++ ")") $ return ()
              doBottom
