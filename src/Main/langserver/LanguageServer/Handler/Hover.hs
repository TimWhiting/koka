...
import Core.FlowAnalysis.DDMCFA.Syntax (runEvalQueryFromRangeSource)
import Core.FlowAnalysis.DDMCFA.DemandMonad (AnalysisKind (..))
             ...
             liftMaybe (return (rangeMapFindAt lexemes pos rmap)) $ \(rng,rngInfo) ->
              ...
              do penv <- getPrettyEnvFor modname
                 ...
                 !res <- liftIO $ trace ("Running eval for position " ++ show pos) $ 
                           do
                            (do 
                              res <- runEvalQueryFromRangeSource 
                                buildContext (\bc mod -> (runBuild term flags (buildcTypeCheck [mod] bc))) (rng, rngInfo) 
                                (fromJust mod) BasicEnvs (callSensitivity options) (debugAnalysis options) (analysisGas options)
                              return $ Right res)  
                            `catchError`
                              (\e -> return (Left e))
