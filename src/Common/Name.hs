...
isSymbolId s
  = case s of
      ...
      (c:cs) -> not (isIdStartChar c && isIdEndChar (last cs) && all isIdChar (init cs))
      "" -> False
...
postpend post name | isSymbolName name
  = -- we must always end in symbols for operators so postpend inserts before the symbols
    ...
    let (rsyms,rid) = span isSymbolChar (reverse stem)
        prefix = reverse rid
        -- ensure prefix is valid; note: we don't use ensureLowerId here as we don't want to drop trailing @
        validPrefix = if null prefix || not (isLowerId prefix) then "@x" ++ prefix else prefix
