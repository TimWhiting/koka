...
ppParams (ValueBinder _ _ (S.Lam pars expr defTop range) _ _)
  = tupled (map ppValBinder pars) <--> indent 2 (ppSyntaxExpr expr)
ppParams (ValueBinder _ _ (S.Ann (S.Lam pars expr defTop range) _ _) _ _)
  = tupled (map ppValBinder pars) <--> indent 2 (ppSyntaxExpr expr)
...
ppSyntaxExpr (S.Lam pars expr topDef range) =
