          ...
          <- try $ do (vis,vrng) <- visibility dvis
                      ...
                      (do (rng,doc) <- dockeyword "val" -- return (vis,vrng,rng,doc,inline,True)
                       ...
                       do fip    <- parseFip
                          ...
                          annotation <- optionMaybe $ do
                              specialId "align"
                              value <- stringLit
                              return value
                          (rng,doc) <- dockeywordFun  -- return (vis,vrng,rng,doc,inline,False)
                          return (funDecl toplevel (combineRange vrng rng) doc vis inline fip (fmap fst annotation))
                       <|>
                       do keyword "fn"
...
funDecl toplevel rng doc vis inline fip annotation
  = do spars <- squantifier
       ...
       return (Def (ValueBinder name () (ann fun) nameRng (combineRange nameRng parsRng)) (combineRanged rng fun) vis
                       (defFunEx pinfos fip) inline doc annotation)
