                    ...
                    -> -- fix it up by adding the polymorphic type application
                       do assumedTpX <- normalize True sassumedTp -- resTp0
                          ...
                          let -- coreX = simplify expr -- coref0 (Core.defExpr coreDef)
                              ...
                              resCoreX = (CoreVar.|~>) [(Core.TName ({- unqualify -} name) assumedTpX (Just rng),
                        ...
                        {-
                          let resCore2 = Core.TypeLam tvars ((CoreVar.|~>) [(Core.TName (unqualify name) resTp1 (Just rng), Core.TypeApp (Core.Var (Core.TName (unqualify name) (resTp1)) Core.InfoNone) (map TVar tvars))] expr)
                          trace ("\n ~> \n" ++ show resCore2) $
                            return resCore2
                        -}
                  _ | divergent  -- we added a divergent effect, fix up the occurrences of the assumed type
                    -> -- trace "  divergent" $
                       do assumedTpX <- normalize True assumedTp >>= subst -- resTp0
                          ...
                          let resCoreX = (CoreVar.|~>) [(Core.TName ({- unqualify -} name) assumedTpX (Just rng), Core.Var (Core.TName ({- unqualify -} name) resTp1 Nothing) info)] coreX
