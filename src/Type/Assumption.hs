...
coreExprFromNameInfo qname info rng
  = -- trace ("create name: " ++ show qname) $
    case info of
      InfoVal vis cname tp _ _ _ _ _         -> Core.Var (Core.TName cname tp (Just rng)) (Core.InfoNone)
      InfoFun vis cname tp _ ((m,n)) _ _ _   -> Core.Var (Core.TName cname tp (Just rng)) (Core.InfoArity m n)
