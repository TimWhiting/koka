...
parcExpr expr
  = case expr of
      App (Var name _) _ _ | getName name == nameLazyTarget
        -> do return expr
      App (Var name varInfo) [var, conApp] rng | getName name == nameLazyUpdate
        -> do conApp' <- parcExpr conApp
              return $ App (Var name varInfo) [var, conApp'] rng
