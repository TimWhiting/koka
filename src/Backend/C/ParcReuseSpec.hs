...
genConTagScanFieldsAssign resultType conName conRepr reuseName tag scan fieldExprs
  = App (Var (TName nameConTagScanFieldsAssign typeConFieldsAssign Nothing) (InfoArity 0 (length fieldExprs + 2)))
        ([Var reuseName (InfoConField conName conRepr nameNil),
          Lit (LitInt (toInteger tag)),
          Lit (LitInt (toInteger scan))] ++ map snd fieldExprs) Nothing
  where
...
genConTagFieldsAssign resultType conName conRepr reuseName tag fieldExprs
  = App (Var (TName nameConTagFieldsAssign typeConFieldsAssign Nothing) (InfoArity 0 (length fieldExprs + 1)))
        ([Var reuseName (InfoConField conName conRepr nameNil), Lit (LitInt (toInteger tag))] ++ map snd fieldExprs) Nothing
  where
    fieldTypes = [(name,typeOf expr) | (name,expr) <- fieldExprs]
