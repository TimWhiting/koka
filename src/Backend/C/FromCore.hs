...
genAppNormal (Var (TName conTagScanFieldsAssign typeAssign _) _) (Var reuseName (InfoConField conName conRepr nameNil):(Lit (LitInt tag)):(Lit (LitInt scan)):fieldValues) | conTagScanFieldsAssign == nameConTagScanFieldsAssign
...
genAppNormal (Var (TName conTagFieldsAssign typeAssign _) _) (Var reuseName (InfoConField conName conRepr nameNil):(Lit (LitInt tag)):fieldValues) | conTagFieldsAssign == nameConTagFieldsAssign
  = do tmp <- genVarName "con"
       let setTag = tmp <.> text "->_base._block.header.tag = (kk_tag_t)" <.> parens (pretty tag) <.> semi
           fieldNames = case splitFunScheme typeAssign of
