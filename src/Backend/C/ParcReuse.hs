...
genDropReuse tname scan
  = App (Var (TName nameDropReuse funTp) (InfoExternal [(C CDefault, ("drop_reuse(#1,#2,kk_context())",False))]))
...
genIsUnique tname
  = App (Var (TName nameIsUnique funTp) (InfoExternal [(C CDefault, ("is_unique(#1)",False))]))
        [Var tname InfoNone]
...
genFree :: TName -> Expr
genFree tname
  = App (Var (TName nameFree funTp) (InfoExternal [(C CDefault, ("kk_constructor_free(#1,kk_context())",False))]))
        [Var tname InfoNone]
...
genReuseDrop tname
  = App (Var (TName nameReuseDrop funTp) (InfoExternal [(C CDefault, ("kk_reuse_drop(#1,kk_context())",False))]))
        [Var tname InfoNone]
...
genReuseNull :: Expr
genReuseNull
  = App (Var (TName nameReuseNull funTp) (InfoExternal [(C CDefault, ("kk_reuse_null",False))])) []
  where funTp = TFun [] typeTotal typeReuse

-- Generate a reuse a block
genReuseAddress :: TName -> Expr
genReuseAddress tname
  = App (Var (TName nameReuse funTp) (InfoExternal [(C CDefault, ("reuse_datatype(#1,kk_context())",False))])) [Var tname InfoNone]
  where
    tp    = typeOf tname
...
genReuseAssignWith reuseName arg
  = let assign = TName nameAssignReuse (TFun [(nameNil,typeReuse),(nameNil,typeReuse)] typeTotal typeUnit)
    in App (Var assign (InfoExternal [(C CDefault, ("#1 = #2",False))])) [Var reuseName InfoNone, arg]
