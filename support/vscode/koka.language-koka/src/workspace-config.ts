...
export class KokaConfig {
  ...
  refreshConfig(): void {
    this.enableDebugExtension = vsConfig.get('dev.debugExtension') as boolean || vsConfig.get('analysis.debug') as boolean    this.cwd = expandHome(this.vsConfig.get('languageServer.workingDirectory')) as string
    if (!this.cwd) {
