  ...
  describe "Common.Name" $ do
    ...
    describe "round-trip tests for constructor name patterns" $ do
      ...
      let constructorPatterns = 
            [ ("@Hnd- from toHandlerName", makeHiddenName "Hnd")
            , ("@Op- from toOpConName", makeHiddenName "Op")
            , ("@Ops- from toOpsConName", makeHiddenName "Ops")
            , ("@tag- from toOpenTagName", makeHiddenName "tag")
            , ("@val- from toValueOperationName", makeHiddenName "val")
            , ("@extern- from newHiddenExternalName", makeHiddenName "extern")
            , ("@create- from newCreatorName", makeHiddenName "create")
            , ("@op- from toOpTypeName", makeHiddenName "op")
            , ("@singleton- from conSingletonNameX", makeHiddenName "singleton")
            ]
          testRoundTrip label op nameStr = 
            it (label ++ " with " ++ nameStr) $ do
              let n1 = readQualifiedName nameStr
